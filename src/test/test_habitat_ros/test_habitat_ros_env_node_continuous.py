...
class HabitatROSEnvNodeContinuousCase(unittest.TestCase):
    ...
    def test_env_node_continuous(self):
        ...
        env_node_args = shlex.split(
            f"python src/nodes/habitat_env_node.py --node-name {self.env_node_under_test_name} --task-config {TestHabitatROSData.test_acts_and_obs_continuous_task_config} --enable-physics-sim --sensor-pub-rate {self.env_pub_rate}"
        )
        Popen(env_node_args)
        ...
        dict_of_metrics = mock_evaluator.evaluate(
            str(int(TestHabitatROSData.test_acts_and_obs_continuous_episode_id) - 1),
